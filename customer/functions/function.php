...
function add_cart()
{
    ...
    if (isset($_GET['add_cart'])) 
    {
	    ...
	    if(mysqli_num_rows($run_check>0)) 
	    {
	       ...
	       echo "<script>window.open('../details.php?proid=$p_id','_self')</script>";
	    }
	    else
	    {
...
function confirm_payments()
{
         ...
         if (isset($_POST["confirm"])) 
  {
    ...
    $complete='complete';
    $insert_into_payments="INSERT INTO payments (invoice_no,amount,payment_mode,ref_no,code,payment_date) VALUES ('$invoice_no','$amount','$payment_mode','$ref_no','$code','$payment_date')";
    $run_insert_into_payments=mysqli_query($con,$insert_into_payments);
    if ($run_insert_into_payments) 
    {
      echo "<script>alert('ok')</script>";
      
    }
    $update_customer_order="UPDATE customer_orders SET order_status='$complete' WHERE order_id='$update_id'";
    $run_update_customer_order=mysqli_query($con,$update_customer_order);
    $update_pending_orders="UPDATE pending_orders SET  order_status='$complete' WHERE order_id='$update_id'";
    $run_update_pending_orders=mysqli_query($con,$update_pending_orders);
    if ($run_update_pending_orders) 
    {
      echo "<script>alert('Thank You For Perchasing')</script>";
...
function customer_edit_account()
{
	global $con;
